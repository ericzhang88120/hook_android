...
static char last_error_string[error_length] = {0};    
...
int remote_mmap_call(pid_t target_pid, struct pt_regs* p_regs,
                     ...
                     void** result)
{
    const size_t parameters_number = 6;
    
    long parameters[parameters_number] = {0};
...
#endif    
    if(ptrace_call_wrapper(target_pid, "mmap", mmap_addr, parameters, parameters_number, p_regs) == -1)
    {
...
#endif    
    if(ptrace_call_wrapper(target_pid, "dlopen", dlopen_addr, parameters, parameters_number, p_regs) == -1)
    {
#if DSH_DEBUG_LOGGER == 1
...
#endif      
    if(ptrace_call_wrapper(target_pid, "dlsym", dlsym_addr, parameters, parameters_number, p_regs) == -1)
    {
#if DSH_DEBUG_LOGGER == 1
...
int remote_dlerror_call(pid_t target_pid, struct pt_regs* p_regs,
                        void** result)
{
    static char error_message[error_length] = {0};
